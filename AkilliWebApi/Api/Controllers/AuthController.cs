...
namespace AkilliWebApi.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public IActionResult CurrentUserInfo(string Username, string Password)
        {
            try
            {
                ...
                string SQL = @"SELECT 
                                  prs.*,
                                  itn.ihale_cari_ad_unvan,
                                  itn.ihale_cari_kodu,
                                  ihs.ihale_hastane_cari_kodu,
                                  ihs.ihale_hastane_cari_ad_unvan,
	                              ch.cari_kodu as personel_cari_kodu,
  	                              ch.cari_ad_unvan as personel_cari_ad_unvan
                                FROM
                                  dbo.tbl_personel prs
                                  LEFT OUTER JOIN dbo.tbl_ihale_tanim itn ON (prs.masraf_yeri = itn.ihale_cari_kodu)
                                  LEFT OUTER JOIN dbo.tbl_ihale_hastane ihs ON (prs.departman = ihs.ihale_hastane_cari_kodu)
                                  LEFT OUTER JOIN dbo.tbl_cari_hesap ch ON (prs.cari_kodu = ch.cari_kodu)
                                WHERE ad_soyad='" + Username.ToString().Trim() + "' AND sifre='" + Password.ToString().Trim() + "' ";
                var db_rows = cnn.Query<AkilliWebApi.Data.RadyolojiTakip.personel>(SQL);
                return Ok(db_rows);
            }
            catch(Exception ex)
            {
